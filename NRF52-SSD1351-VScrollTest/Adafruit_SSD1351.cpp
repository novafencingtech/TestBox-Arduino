...
static const uint8_t PROGMEM initList[] = {
  ...
  SSD1351_CMD_DISPLAYOFFSET, 1,
    0x0,
  //SSD1351_CMD_DISPLAYENHANCE, 3, 0xA4,0x00,0x00, //Display enhancement mode
...
/**************************************************************************/
/*!
 @brief  Enable horiztonal scrolling
 @param  enable True if you want horizontal scrolling, false
 @param  dir  (0x01 or 0xff)
 @param startRow start row address 
 @param numRows number of rows to be H-scrolled B+C <= 128
 @param speed scrolling time interval 00b test mode 01b normal 10b slow 11b slowest
 */
/**************************************************************************/

void Adafruit_SSD1351::setHorizontalScroll(bool enable, uint8_t dir, uint8_t startRow,uint8_t numRows,uint8_t scrollSpeed) {
  
  if (enable==false) {
    startWrite();    
    writeCommand(SSD1351_CMD_HORIZSCROLL);
    spiWrite(0x0);
    spiWrite(0x0); //B[6:0] : start row address 
    spiWrite(0x0); //C [7:0] : number of rows to be H-scrolled B+C <= 128
    spiWrite(0x00); //D[6:0] : Reserved (reset=00h) 
    spiWrite(0x0); //E[1:0] : scrolling time interval 00b test mode 01b normal 10b slow 11b slowest
  ...
  writeCommand(SSD1351_CMD_HORIZSCROLL);
  spiWrite(dir); //A 
  spiWrite(startRow); //B[6:0] : start row address 
  spiWrite(numRows); //C [7:0] : number of rows to be H-scrolled B+C <= 128
  spiWrite(0x00); //D[6:0] : Reserved (reset=00h) 
  spiWrite(scrollSpeed); //E[1:0] : scrolling time interval 00b test mode 01b normal 10b slow 11b slowest

  writeCommand(SSD1351_CMD_STARTSCROLL);
  endWrite();
}

void Adafruit_SSD1351::setDisplayStartLine(uint8_t val) {
  startWrite();
...
void Adafruit_SSD1351::setDisplayOffset(uint8_t val) {
  startWrite();
...
void Adafruit_SSD1351::setMuxRatio(uint8_t val) {
  startWrite();
