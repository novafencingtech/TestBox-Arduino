...
void barGraph(int X, int H, int newVal, int &oldVal, int oThresh, int gThresh, String conn) {
  ...
  if (newVal > 320) {
    tft.fillRect(0,X+16, 128-16, H, BLACK);
...
void updateOLED(char Mode) {
  static int i = 0, val, oldA = 0, oldB = 0, oldC = 0, oldMode = 'z';
  tft.setCursor(0, 0);
  tft.setTextColor(YELLOW, BLACK); tft.setTextSize(2);
  if (Mode != oldMode) {
    Serial.print("oldMode:");Serial.print(oldMode);Serial.print(" newMode:");Serial.print(Mode); Serial.println("/");
    oldMode = Mode;
    switch (Mode) {
      case 'c':
        tft.fillRect(0, 0, 128, 128, BLACK);
        tft.setCursor(0, 0);
        tft.setTextColor(YELLOW, BLACK); tft.setTextSize(2); tft.println("Cable");
        break;
      case 'w':
      case 'r':
        break;
  ...
  switch (Mode) {
    case 'c':
      barGraph(30, 8, grahamToBrian(cableState.ohm_AA), oldA, 100, 50, "A");
      barGraph(65, 8, grahamToBrian(cableState.ohm_BB), oldB, 100, 50, "B");
      barGraph(100, 8, grahamToBrian(cableState.ohm_CC), oldC, 100, 50, "C");
      break;
    case 'w':
    case 'r':
      break;
